...
**What happens on WIN-DC01:**

Microsoft Edge downloads the file `ds7002.lnk` to the Downloads folder. Sysmon logs the process creation and Wazuh captures the event.
...
Rundll32 is used to execute malicious code in a way that appears to be a legitimate system operation, avoiding detection by solutions that only block direct PowerShell.
...
The attacker compresses the collected data using 7-Zip installed on the system to prepare for exfiltration.
...
The attacker downloads additional tools to the compromised system using PowerShell Invoke-WebRequest.
...
To ensure access persists after a reboot, the attacker copies a payload to the user Startup folder. The next time the user logs in the payload executes automatically.
...
After the full attack execution the SIEM recorded 188 alerts distributed across the 12 detection rules created via API.

To view the evidence in your lab open Kibana at `http://SOC-CORE:5601` and navigate to:
